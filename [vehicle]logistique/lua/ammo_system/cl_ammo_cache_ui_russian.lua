if CLIENT then
    local COLORS = {
        ...
        success = Color(60, 90, 40, 255),
        warning = Color(160, 60, 40, 255),
        border = Color(15, 25, 10, 255),
        highlight = Color(200, 180, 140, 255),
        soviet = Color(200, 30, 30, 255),
    ...
    local EMBLEM = Material("vgui/russian_armed_logo.png", "smooth")
    ...
    local function CreateRussianButton(parent, x, y, w, h, text, icon, onClick, enabled)
        ...
        button.Paint = function(self, w, h)
            ...
            draw.SimpleText(text, "RussianButton", textX, textY, COLORS.text, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
    ...
    local function CreateDeployMenuUI(vehicle, closeCallback, deployCallback)
        if not IsValid(vehicle) or vehicle:GetClass() ~= "anomalie_russian_transport" then
            return nil
        end
        ...
        frame.Paint = function(self, w, h)
            ...
            draw.SimpleText("TRANSPORT RUSSE", "RussianTitle", w/2, 22, COLORS.text, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            ...
            emblemImage:SetPos((frameW - emblemW) / 2, 145)
            emblemImage:SetMaterial(EMBLEM)
            emblemImage:SetKeepAspect(false)
            emblemImage:SetZPos(10)
        ...
        closeButton:SetFont("RussianButton")
        ...
        local deployButton = CreateRussianButton(frame, 10, 270, 480, 50, "DÉPLOYER UNE CAISSE", nil, deployCallback, count > 0)
        local cancelButton = CreateRussianButton(frame, 10, 330, 480, 50, "ANNULER", ICONS.cancel, function()
            frame:Close()
            if closeCallback then closeCallback() end
        end)
    ...
    AmmoCacheUI = {
        COLORS = COLORS,
        ICONS = ICONS,
        DrawRussianBorder = DrawRussianBorder,
        CreateRussianButton = CreateRussianButton,
